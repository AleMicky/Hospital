...
export function useCatalogoGrupoItems(grupoId: number | null) {
    return useAppQuery({
        queryKey: queryKeys.catalogoGrupos.items(grupoId ?? 0),
...
export function useDeleteCatalogoGrupo() {
    ...
    return useAppMutation({
        mutationFn: (id: number) => catalogoGruposService.delete(id),
        onSuccess: () => {
            void queryClient.invalidateQueries({
                queryKey: queryKeys.catalogoGrupos.all,
            })
