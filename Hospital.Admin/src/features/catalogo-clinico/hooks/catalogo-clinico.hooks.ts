...
export function useAreaDepartamentos(areaId: number | null) {
    return useAppQuery({
        queryKey: queryKeys.catalogoClinico.areas.departamentos(areaId ?? 0),
...
export function useDepartamentoServicios(departamentoId: number | null) {
    return useAppQuery({
...
export function useServicioPrestaciones(servicioId: number | null) {
    return useAppQuery({
        queryKey: queryKeys.catalogoClinico.servicios.prestaciones(servicioId ?? 0),
...
export function useDeletePrestacion() {
    const qc = useQueryClient()
    return useAppMutation({
        mutationFn: (id: number) => prestacionesService.delete(id),
        onSuccess: () => {
            void qc.invalidateQueries({
                queryKey: queryKeys.catalogoClinico.all,
            })
...
type SimpleCatalogConfig = {
    ...
    service: {
        ...
        update: (id: number, data: UpdateCatalogoBasePayload) => ReturnType<typeof especialidadesService.update>
        delete: (id: number) => ReturnType<typeof especialidadesService.delete>
    }
