...
export function useUpdatePaciente() {
    ...
    return useAppMutation({
        mutationFn: ({ id, data }: { id: number; data: UpdatePacientePayload }) =>
...
export function useDeletePaciente() {
    ...
    return useAppMutation({
        mutationFn: (id: number) => pacientesService.delete(id),
        onSuccess: () => {
            void queryClient.invalidateQueries({ queryKey: queryKeys.pacientes.all })
            notify.success('Paciente desactivado', 'El paciente se desactivó correctamente.')
        },
        onError: (error) => {
            notify.error('Error al desactivar paciente', getApiErrorMessage(error))
        },
    })
}
